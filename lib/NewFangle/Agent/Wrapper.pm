...

{
    ...
    *CORE::GLOBAL::caller = sub (;$) {
        ...
        while (1) {
            ...
            my $is_wrapper
                =  $caller[0] eq 'NewFangle::Agent::Wrapper'
                || $caller[0] eq 'Hook::LexWrap';

            $name_cache = $is_wrapper ? $caller[3] : '';
...
sub wrap (*@) {  ## no critic Prototypes
    ...
    Carp::croak "'$_' value is not a subroutine reference"
        for grep {$wrapper{$_} && ref $wrapper{$_} ne 'CODE'} qw( pre post );
    ...
    return bless sub{ $unwrap=1 }, 'NewFangle::Agent::Wrapper::Cleanup';
}

package
    NewFangle::Agent::Wrapper::Cleanup;
...
NewFangle::Agent::Wrapper - A local fork of Hook::LexWrap
...
B<NOTE>: This is a local fork of Hook::LexWrap that uses L<Syntax::Keyword::Defer>
to ensure that C<post> wrappers I<always> run, even if the wrapped function dies.
The documentation below is copied verbatim from the source module, with only the
module names changed).

NewFangle::Agent::Wrapper allows you to install a pre- or post-wrapper (or both)
around an existing subroutine. Unlike other modules that provide this
capacity (e.g. Hook::PreAndPost and Hook::WrapSub), NewFangle::Agent::Wrapper
...
$_[-1] may be assigned to in a pre-wrapper, in which case NewFangle::Agent::Wrapper
