...
set either during import or when caling L<initialize|/initialize> (see below).

This value is optional, but if set, it I<must> point to a readable file.

If not set, no configuration file will be read.

The location of this file can be overriden with the C<NEWRELIC_CONFIG_FILE>
...
The name of the environment can be overriden with the C<NEWRELIC_ENVIRONMENT>
environment variable.
...
Can be overriden with the C<NEWRELIC_ENABLED> environment variable.

=head2 app_name

String, must be set.

Can be overriden with the C<NEWRELIC_APP_NAME> environment variable.

=head2 license_key

String, must be set.

Can be overriden with the C<NEWRELIC_LICENSE_KEY> environment variable.
...
Can be overriden with the C<NEWRELIC_LOG_FILE> environment variable.
...
production, and testing environments to suiot different needs.
...
This is etirely optional. If an environment is set, its values will be read
...
This can be overriden with the C<NEWRELIC_ENVIRONMENT> environment variable.
