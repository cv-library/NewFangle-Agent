...
The code that starts these segments is generated with the C<generate_segment_starter>
function, described in more detail L<below|/generate_segment_starter>.

=head1 FUNCTIONS

=head2 generate_segment_starter
...
called. The code reference should I<always> return a L<NewFangle::Segment>
object.

If C<generate_segment_starter> returns an undefined value, a default tracking
...
See L</Adjusting the monitoring segments> above for a discussion of this
process.
...
C<transaction_tracer.include> and C<transaction_tracer.exclude> control what
...
except the code loaded from C<lib/Local/Secret>, will be considered relevant.
